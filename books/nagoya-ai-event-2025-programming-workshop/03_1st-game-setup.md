...
/**
 * @typedef {Awaited<ReturnType<typeof renderMap>>} MapContext
 */
...
1.  **game.js**: 3D マップを表示、ゲーム全体をコントロールするファイル
2.  **game-model.js**: 3D のモデリングを行うファイル
3.  **ai-fetch.js**: AI と通信するためのファイル
4.  **ai-system-prompt.js**: AI の設定を記述するファイル
...
#### パターン１：コードをしっかり開けていない

**原因**: 「ファイルの中身（コピー&ペースト）」と書かれた部分をクリックして、コードが全部表示されているか確認できていない。

**対処法**: 「ファイルの中身（コピー&ペースト）」と書かれた部分をクリックし、コードが全部表示されていることを確認してください。
...
:::details B. コードを間違えて編集してしまった

**原因**: 意欲的な生徒がコードを理解しようとして、誤って編集してしまった。

**対処法**: 素晴らしい探求心だと褒めた上で、「まずは一度、元のコードで動かすところまでやってみよう！」と促します。該当ファイルのコードを一度全て削除し、再度マニュアルからコピー＆ペーストして元の状態に戻します。変更して試したい箇所は、後で自由に試せる時間があることを伝えると良いでしょう。
