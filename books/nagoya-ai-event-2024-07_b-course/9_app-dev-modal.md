...
1. リストがクリックされた時にモーダルを表示する
2. モーダルの外側がクリックされた時にモーダルを閉じる
...
記述が完了したら、ファイルを保存し Live Server で http://localhost:5500 にアクセスして、料理のレシピをクリックしたときにモーダルが表示されることを確認しましょう！！

![デモ２](https://github.com/itnav/zenn-gura/blob/main/books/nagoya-ai-event-2024-07_b-course/assets/9_app-dev-modal/demo.gif?raw=true)

::: details 【深く知りたい人向け】レシピを表示する方法

#### 1. リストがクリックされた時にモーダルを表示する

リストがクリックされた時にモーダルを表示するためには、まず、リストの要素にイベントリスナーを追加します。

```javascript
// リストクリック時にモーダルを表示するイベントハンドラを追加する
recipeItemElement.addEventListener('click', () => {
  // ...
});
```

その後、モーダルの中身を置き換えるために、`recipeModalElement.innerHTML` に HTML を追加します。
...
モーダルの外側がクリックされた時にモーダルを閉じるためには、モーダルの要素にイベントリスナーを追加します。

```javascript
// モーダルがクリックされた時に第２引数の関数を実行するように設定
recipeModalElement.addEventListener('click', function onClick(event) {
  // モーダル本体がクリックされたとき、つまり、モーダルの子要素がクリックされなかったとき、モーダルを閉じる
  if (recipeModalElement === event.target) {
    recipeModalElement.classList.remove('open');
    recipeModalElement.removeEventListener('click', onClick);
  }
});
```
